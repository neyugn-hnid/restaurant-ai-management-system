...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function loadTables() {
        try {
            ...
            return []; // Không dùng localStorage cache
        }
    }
    ...
    elements.confirmBtn.addEventListener('click', async () => {
        ...
        document.getElementById('tableDetailModal').classList.remove('active');
        const oldPreorders = JSON.parse(localStorage.getItem('bistro_pending_preorders') || '[]');
        const cleanedPreorders = oldPreorders.filter(p => String(p.tableId) !== String(selectedTableId));
        localStorage.setItem('bistro_pending_preorders', JSON.stringify(cleanedPreorders));
        localStorage.setItem('bistro_customer_cart', JSON.stringify([]));
        localStorage.removeItem('bistro_tables');  // Xóa cache bảng cũ!
        const orderNowBtn = document.getElementById('orderNowBtn');
        if (orderNowBtn) {
            orderNowBtn.href = `./customer-menu.html?table=${selectedTableId}`;
        }
