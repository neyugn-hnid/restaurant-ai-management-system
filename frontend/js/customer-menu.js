...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    document.getElementById('checkout-btn').addEventListener('click', async () => {
        ...
        const isPreorder = hasActiveReservation(cTable);
        ...
        if (isPreorder) {
            ...
            await syncPreorderToReservation(cTable, preorderInfo);
            msgEl.innerHTML = `Cảm ơn <b>${cName}</b>.<br/>Món đã được ghi nhận. Hóa đơn sẽ được tạo khi nhà hàng nhận bàn.`;
        } else {
            ...
            msgEl.innerHTML = `Cảm ơn <b>${cName}</b>.<br/>Đơn hàng đã được gửi lên hệ thống.`;
        }
        const modal = document.getElementById('order-success-modal');
